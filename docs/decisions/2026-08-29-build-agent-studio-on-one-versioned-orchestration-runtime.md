...
- **Decision:** Agent Studio stores schema-versioned definitions for four compositions over the existing executor: Standard Agent, Goal Loop, Coordinator Team, and acyclic Workflow. Built-in templates are immutable starting points; saved definitions are validated before persistence. One GInfer model serves the composition, parallel roles are bounded by its supported concurrency, child stages have isolated session state, and parent cancellation and approval policy govern the entire run.
