...
- [ ] Add 2 custom assistants to GChat 
...
- [ ] [0.6.9] Take a `gguf` file and delete the `.gguf` extensions from the file name, import it into GChat and verify that it works.
...
- [ ] [0.6.9] Make sure that Ollama set-up  as a custom provider work with GChat
...
- [ ] [0.6.9] Make sure that you can send an image to a Local API Server and it also works (can set up Local API Server as a Custom Provider in GChat to test)
- [ ] [0.6.10] Make sure you are still able to see API key when server local status is running
- [ ] [0.7.0] Users can see the GChat API Server Swagger UI by opening the following path in their browser `http://<ip>:<port>`
...
- [ ] Clicking on a listing will open up the model card information within GChat and render the HTML properly
...
## E. Assistants
- [ ] There is always at least one default Assistant which is GChat
- [ ] The default GChat assistant has `stream = True` by default 
...
- [ ] Click on `Reset` button in `Other` to factory reset the app:
	- [ ] All threads deleted
	- [ ] All Assistant deleted except for default GChat Assistant
...
- [ ] Clean up by deleting all the left over folder created by GChat
	- [ ] On MacOS
		- [ ] `~/Library/Application Support/GChat`
		- [ ] `~/Library/Caches/app.gchat`
	- [ ] On Windows
		- [ ] `C:\Users<Username>\AppData\Roaming\GChat\`
		- [ ] `C:\Users<Username>\AppData\Local\app.gchat`
	- [ ] On Linux
		- [ ] `~/.cache/GChat`
		- [ ] `~/.cache/gchat`
		- [ ] `~/.local/share/GChat`
		- [ ] `~/.local/share/app.gchat`
- [ ] Ensure that the fresh install of GChat launch
