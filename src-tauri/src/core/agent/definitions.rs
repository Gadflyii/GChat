...
pub fn list_definitions(data_folder: &Path) -> Result<Vec<AgentDefinition>, String> {
    let mut definitions = vec![general_agent()];
    definitions.extend(read_store(data_folder)?.definitions);
...
mod tests {
    ...
    fn persists_custom_definitions_and_keeps_general_builtin() {
        ...
        assert_eq!(list_definitions(root.path()).unwrap().len(), 1);
    }
    ...
    fn rejects_shared_workspace_stages_in_parallel_levels() {
        ...
        let error = validate_workflow(
            &nodes,
            &[edge("write", "deliver"), edge("read", "deliver")],
        )
        .unwrap_err();
