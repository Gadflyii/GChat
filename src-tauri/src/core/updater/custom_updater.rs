/**
 * Custom Updater for GChat with HMAC request signing
 *
...
/// - In CI: Set GCHAT_SIGNING_KEY environment variable at build time
/// - In local dev: Falls back to a test key
const SECRET_KEY: &str = match option_env!("GCHAT_SIGNING_KEY") {
    Some(key) => key,
...
impl CustomUpdater {
    ...
    /// Build User-Agent header: GChat/{version} ({os}; {arch})
    fn build_user_agent(app_version: &str) -> String {
        let os = std::env::consts::OS;
        let arch = std::env::consts::ARCH;
        format!("GChat/{} ({}; {})", app_version, os, arch)
    }
