...
def get_gchat_log_paths(is_nightly=False):
    """
    Get GChat application log file paths based on OS and version (nightly vs regular)
    Returns list of glob patterns for log files
    """
    system = platform.system().lower()
    app_name = "GChat-nightly" if is_nightly else "GChat"
    
    if system == "windows":
        # Windows: %APPDATA%\GChat(-nightly)\data\logs\*.log
        appdata = os.path.expandvars("%APPDATA%")
        return [f"{appdata}\\{app_name}\\data\\logs\\*.log"]
    
    elif system == "darwin":  # macOS
        # macOS: ~/Library/Application Support/GChat(-nightly)/data/logs/*.log
        home_dir = os.path.expanduser("~")
        return [f"{home_dir}/Library/Application Support/{app_name}/data/logs/*.log"]
    
    elif system == "linux":
        # Linux: ~/.local/share/GChat(-nightly)/data/logs/*.log
        home_dir = os.path.expanduser("~")
...
def upload_gchat_logs(client, test_item_id, is_nightly=False, max_log_files=5):
    """
    Upload GChat application log files to ReportPortal
    """
    log_patterns = get_gchat_log_paths(is_nightly)
    app_type = "nightly" if is_nightly else "regular"
    
    logger.info(f"Looking for GChat {app_type} logs...")
    ...
    if not all_log_files:
        logger.warning(f"No GChat {app_type} log files found")
        client.log(
            time=timestamp(),
            level="WARNING",
            message=f"[INFO] No GChat {app_type} application logs found",
    ...
    try:
        ...
        logger.info(f"Uploading {len(log_files_to_upload)} most recent GChat {app_type} log files")
        ...
        for i, log_file in enumerate(log_files_to_upload, 1):
            try:
                ...
                client.log(
                    time=timestamp(),
                    level="INFO",
                    message=f"[INFO] GChat {app_type} application log: {file_name}",
                    item_id=test_item_id,
                    attachment={
                        "name": f"gchat_{app_type}_log_{i}_{file_name}",
        ...
        client.log(
            time=timestamp(),
            level="INFO",
            message=f"[INFO] Uploaded {len(log_files_to_upload)} GChat {app_type} log files (total available: {len(all_log_files)})",
            item_id=test_item_id
        )
        
    except Exception as e:
        logger.error(f"Error processing GChat logs: {e}")
        client.log(
            time=timestamp(),
            level="ERROR",
            message=f"Error processing GChat {app_type} logs: {str(e)}",
            item_id=test_item_id
        )
...
def upload_test_results_to_rp(client, launch_id, test_path, trajectory_dir, force_stopped=False, video_path=None, is_nightly=False):
    ...
    try:
        ...
        # Upload GChat application logs
        logger.info("Uploading GChat application logs...")
        upload_gchat_logs(client, test_item_id, is_nightly=is_nightly, max_log_files=5)
