#!/bin/bash
# macOS download script for GChat app
...
# 2. Repository variable GCHAT_APP_URL
...
curl -L -o "/tmp/gchat-installer.dmg" "$GCHAT_APP_URL"

if [ ! -f "/tmp/gchat-installer.dmg" ]; then
    echo "[FAILED] Failed to download GChat app"
    exit 1
fi

echo "[SUCCESS] Successfully downloaded GChat app"
ls -la "/tmp/gchat-installer.dmg"
