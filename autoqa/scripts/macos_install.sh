#!/bin/bash
# macOS install script for GChat app

echo "Installing GChat app from DMG..."

# Mount the DMG
hdiutil attach "/tmp/gchat-installer.dmg" -mountpoint "/tmp/gchat-mount"

# Find the .app file in the mounted DMG
APP_FILE=$(find "/tmp/gchat-mount" -name "*.app" -type d | head -1)

if [ -z "$APP_FILE" ]; then
    echo "[Failed] No .app file found in DMG"
    hdiutil detach "/tmp/gchat-mount" || true
    exit 1
fi
...
# Unmount the DMG
hdiutil detach "/tmp/gchat-mount"
...
POTENTIAL_EXECUTABLES=(
    ...
    "/Applications/$APP_NAME/Contents/MacOS/GChat"
    "/Applications/$APP_NAME/Contents/MacOS/GChat-nightly"
)
...
echo "GCHAT_APP_PATH=$APP_PATH" >> $GITHUB_ENV
...
# Verify installation
if [ -f "$APP_PATH" ]; then
    echo "[SUCCESS] GChat app installed successfully"
    ls -la "/Applications/$APP_NAME"
else
    echo "[FAILED] GChat app installation failed - executable not found"
    exit 1
fi
